...
class Broadcast @Inject()(dfService: DataframeService, override val resultStorage: ResultStorage)
  extends AsyncDataframeOperation[DataframeRequest] {

  override protected def runImpl(requestEntity: DataframeRequest)
                                (implicit ec: ExecutionContext): Future[Dataframe] = Future {
    dfService.broadcast(requestEntity.df)
  }
}