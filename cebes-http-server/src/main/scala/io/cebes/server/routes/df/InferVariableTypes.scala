...
/**
  * Automatically infer variable types for the given dataframe, using some heuristic\
  */
class InferVariableTypes @Inject()(dfService: DataframeService, override val resultStorage: ResultStorage)
  extends AsyncDataframeOperation[LimitRequest] {

  override protected def runImpl(requestEntity: LimitRequest)
                                (implicit ec: ExecutionContext): Future[Dataframe] = Future {
    dfService.inferVariableTypes(requestEntity.df, requestEntity.n)
  }
}
