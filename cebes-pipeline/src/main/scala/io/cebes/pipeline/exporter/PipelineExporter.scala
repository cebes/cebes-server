...
class PipelineExporter @Inject()(private val stageExporter: StageExporter) {
  ...
  /**
    ...
    *                        pipeline will be serialized in. If None, use system default value
    *                        (see [[io.cebes.pipeline.factory.ModelFactory]] for more information.
    * @return
    */
  def export(ppl: Pipeline, modelStorageDir: Option[String])
            (implicit ec: ExecutionContext): Future[PipelineExportDef] = {
    val futureStageDefs = ppl.stages.values.map { stage =>
      stageExporter.export(stage, modelStorageDir)
    }

    Future.sequence(futureStageDefs).map { stageDefs =>
      PipelineExportDef(PipelineExporter.VERSION, PipelineDef(Some(ppl.id), stageDefs.toArray))
    }
  }
  ...
  def export(ppl: Pipeline, storageDir: String)(implicit jsPplExp: JsonWriter[PipelineExportDef],
    ...
    val modelDir = Files.createDirectory(Paths.get(destDir, PipelineExporter.MODEL_SUB_DIR))

    export(ppl, Some(modelDir.toString)).map { pplExp =>
  ...
  /**
    * Import the given [[PipelineExportDef]] into a [[Pipeline]] object
    *
    * @param pplExp          the pipeline export definition
    * @param modelStorageDir optional, the directory where all the [[io.cebes.pipeline.ml.Model]] in this
    *                        pipeline were serialized in. If None, use system default value
    *                        (see [[io.cebes.pipeline.factory.ModelFactory]] for more information.
    */
  def imports(pplExp: PipelineExportDef, modelStorageDir: Option[String])(implicit ec: ExecutionContext): Pipeline = {
    val pplDef = pplExp.pipeline
    val stageMap = mutable.Map.empty[String, Stage]
    pplDef.stages.map { s =>
      val stage = stageExporter.imports(s, modelStorageDir)
  ...
  def imports(storageDir: String)(implicit jsonReader: JsonReader[PipelineExportDef],
    ...
    val modelDir = Files.createDirectory(Paths.get(storageDir, PipelineExporter.MODEL_SUB_DIR))

    imports(pplExp, Some(modelDir.toString))
  }
}
