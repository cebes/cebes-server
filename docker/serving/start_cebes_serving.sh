...
CEBES_EXPOSED_DIR=${CEBES_EXPOSED_DIR:-"/cebes/serving"}
CEBES_MYSQL_SERVER="127.0.0.1:3306"
MYSQL_OPTIONS="?createDatabaseIfNotExist=true&nullNamePatternMatchesAll=true&useSSL=false"

mkdir -p "${CEBES_EXPOSED_DIR}/spark-warehouse" "${CEBES_EXPOSED_DIR}/logs" "${CEBES_EXPOSED_DIR}/mysql"
...
export CEBES_SPARK_WAREHOUSE_DIR="${CEBES_EXPOSED_DIR}/spark-warehouse"
export CEBES_SERVING_CONFIG_FILE="${CEBES_EXPOSED_DIR}/config.json"

CEBES_JAR=`find /cebes -name cebes-pipeline-serving-assembly-*.jar | head -n 1`

/spark/bin/spark-submit --class "io.cebes.serving.Main" \
    --master "local[*]" \
    --conf "spark.driver.extraJavaOptions=-Dcebes.logs.dir=${CEBES_EXPOSED_DIR}/logs/" \
    ${CEBES_JAR}
